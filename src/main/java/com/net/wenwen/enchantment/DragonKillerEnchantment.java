...
public class DragonKillerEnchantment extends Enchantment {
    public static final TagKey<EntityType<?>> DRAGON_TAG = TagKey.create(
            Registries.ENTITY_TYPE,
            new ResourceLocation("wenwen", "dragons")
    );

    public DragonKillerEnchantment() {
    ...
    public void doPostAttack(LivingEntity attacker, Entity target, int level) {
        if (target instanceof LivingEntity living) {
            if (level > 0) {
                float damage = (10*level)+(level*(living.getMaxHealth()*0.005f));
                /*if (living.getType().is(DRAGON_TAG)) {
                    living.hurt(attacker.damageSources().mobAttack(attacker), damage);
                }*/
                String targetRegistryName = living.getName().getString();
                if (targetRegistryName.contains("龙"))
                {
                    living.hurt(attacker.damageSources().mobAttack(attacker), damage);
                }
            }
